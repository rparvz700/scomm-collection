...
@section('title', 'Audit Logs | SCOMM Collection')

@section('content')
    <div class="page-heading">
        <div>
            <h1>Audit Logs</h1>
            <p>Complete field-by-field audit trail of user edits on Active & Discontinued Monthly Summaries.</p>
        </div>
    </div>

    <div class="panel" style="margin-bottom: 24px;">
        <div class="panel-header" style="flex-wrap: wrap; gap: 14px;">
            <h2>Monthly Summary Field Edits</h2>
            <form method="GET" action="{{ route('logs.audit') }}" style="display: flex; gap: 8px; margin: 0;">
                ...
                @if($search)
                    <a href="{{ route('logs.audit') }}" class="button" style="min-height: 36px; padding: 0 12px; text-decoration: none;">Clear</a>
                @endif
            </form>
        </div>
                ...
                <tbody>
                    @forelse($logs as $log)
                        <tr>
                            ...
                            <td>
                                <span class="pill" style="{{ $log->summary_type === 'active' ? 'background: #dcfce7; color: #166534;' : 'background: #fee2e2; color: #991b1b;' }}">
                                    {{ ucfirst($log->summary_type) }}
                                </span>
                            </td>
                            <td>
                            ...
                            <td style="color: #0268e0; font-weight: 700; font-size: 13px;">{{ $log->new_value ?? '—' }}</td>
                            <td style="font-size: 13px; font-weight: 600;">{{ $log->updated_by ?? 'User' }}</td>
                            <td style="font-size: 12px; color: var(--muted);">{{ $log->ip_address ?? '—' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center; color: var(--muted); padding: 32px;">No summary audit logs recorded yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
