...
foreach ($liveRows as $rowNum => $row) {
    if ($rowNum === 521) {
        ...
        echo "Inside loop for row 521:\n";
        echo "  rawClientName: '$rawClientName'\n";
        echo "  opusId: '$opusId'\n";
        
        if (empty($opusId)) echo "  ERROR: opusId is empty!\n";
        if (empty($rawClientName)) echo "  ERROR: rawClientName is empty!\n";
        if (!ctype_digit($opusId)) echo "  ERROR: ctype_digit failed!\n";
        
        $clientName = $cleanClientName($rawClientName);
        $client = $findClient($clientName, $opusId);
        
        if ($client) {
            echo "  Matched ID {$client->client_id} ({$client->client_name})\n";
        } else {
            echo "  Unmatched!\n";
        }
    }
